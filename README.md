...
An 8-bit chop sampler plugin by **@sorryorri**. Slice loops, play one-shots on 16 pads, add some FX, and level up Sir Slicealot the knight while you play.

Built with [JUCE](https://juce.com). Runs as an Audio Unit, VST3 or standalone app on macOS 11+ (Apple Silicon and Intel).

## Features

- **Slicer**: drop a loop and cut it into an even grid (1–64 slices) or at every detected hit. Slices play from C1 (MIDI note 36) upwards.
